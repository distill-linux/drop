...
static void transform(distill_sha256_ctx *ctx, const uint8_t data[64]) {
...
void distill_sha256_final(distill_sha256_ctx *ctx, uint8_t digest[32]) {
    ...
    distill_sha256_update(ctx, &pad, 1);
    uint8_t zero = 0;
    while ((ctx->count % 64) != 56) {
        distill_sha256_update(ctx, &zero, 1);
    }
    distill_sha256_update(ctx, len_bytes, 8);
...
int distill_sha256_file(const char *path, char hex[65]) {
    ...
    if (!f) return -1;
    distill_sha256_ctx ctx;
    distill_sha256_init(&ctx);
    uint8_t buf[8192];
    size_t n;
    while ((n = fread(buf, 1, sizeof(buf), f)) > 0) {
        distill_sha256_update(&ctx, buf, n);
    }
    fclose(f);
    uint8_t digest[32];
    distill_sha256_final(&ctx, digest);
    distill_sha256_to_hex(digest, hex);
    return 0;
}
